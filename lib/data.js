...
function InsertRequest(host, config) {
  var url = new Url(host, {
    path: resolve("insert", config.entity, config.version)
  });
...
function SaveRequest(host, config) {
  var url = new Url(host, {
    path: resolve("save", config.entity, config.version)
  });
...
function UpdateRequest(host, config) {
  var url = new Url(host, {
    path: resolve("update", config.entity, config.version)
  });
...
function DeleteRequest(host, config) {
  var url = new Url(host, {
    path: resolve(host, "delete", config.entity, config.version)
  });
